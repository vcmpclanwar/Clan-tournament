...
function AccInfo( player )
{
	...
	if( q )
	{
		...
		if( ( player.UID == status[player.ID].UID ) || ( player.IP == status[player.ID].IP ) )
		{
			if( status[player.ID].banned == "Yes" )
			{
				...
				Message( "[#FFDD33]Information:[#FFFFFF] Player: " + player.Name + " was kicked from server | Reason: Banned." );
				KickPlayer( player );
			} 
			else
			{ 
				if ( status[player.ID].AutoLogin == "true" )
				{
					...
					if( status[player.ID].clan != null ) MessagePlayer( "[#FFDD33] Clan: "+ status[player.ID].clan +".", player );
				...
				{
					MessagePlayer("[#FFDD33]Welcome to the [#FFFF00]VCMP Clan Tournament 2018[#FFFFFF]!", player);
					MessagePlayer("[#FFDD33]Information:[#FFFFFF] Your nick is registered. Please login in order to access our services.", player );
				}
			}
		}
		else
	...
	{
		MessagePlayer( "[#FFDD33] Welcome to the [#FFFF00]VCMP Clan Tournament 2018[#FFFFFF]!", player );
		MessagePlayer( "[#FFDD33]Information:[#FFFFFF] Your nick is [#FF0000]not [#FFFFFF]registered. Please use /"+bas+"register[#FFFFFF] in order to access our services.", player );
	}
    //FreeSQLQuery( q );
}
...
function onPlayerRequestSpawn( player )
{
	...
	{
		MessagePlayer( "[#FF0000]Error:[#FFFFFF] You need to be register before spawning.", player );
		return 0;
	}
	...
	else
	{
		...
		{
			MessagePlayer("[#FF0000]Error:[#FFFFFF] You are in Tournament Group 1. You can only spawn with Team Yellow or Blue.", player );
			return 0;
		}
		else if( lvl == 3 && ( player.Team == 2 || player.Team == 4 ) )
		{
			MessagePlayer("[#FF0000]Error:[#FFFFFF] You are in Tournament Group 2. You can only spawn with Team Yellow or Red.", player );
			return 0;
		}
		else if( lvl == 4 && ( player.Team == 2 || player.Team == 3 ) )
		{
			MessagePlayer("[#FF0000]Error:[#FFFFFF] You are a Referee. You can only spawn with Team Yellow or Green.", player );
			return 0;
		}
		else return 1;
	}
...
function onPlayerCommand(player, command, arguments)
{
	...
	if( cmd == "register" )
	{
		...
		else if( arguments.len() < 4) MessagePlayer("[#FF0000]Error:[#FFFFFF] Password should contain at least 4 characters.", player );
		else
		{
	...
	else if( cmd == "login" )
	{
		...
		else
		{
			...
			if( !q ) MessagePlayer("[#FF0000]Error:[#FFFFFF] There is a problem with your account. Please contact the developer of the server.", player )
			else
			{
	...
	else if( cmd == "credits" )
	{
		...
		else if( !status[player.ID].Registered ) MessagePlayer( "[#FF0000]Error:[#FFFFFF] You need to register first. Use /"+bas+"register", player );
		else if( !status[player.ID].LoggedIn ) MessagePlayer( "[#FF0000]Error:[#FFFFFF] You need to login first. Use /"+bas+"login", player );
	}
	...
	else if( cmd == "changepass" || cmd == "changepassword" )
	{
		...
		else if( GetTok( arguments, " ", 2).len() < 4 ) MessagePlayer( "[#FF0000]Error:[#FFFFFF] New password should contain at least 4 characters.", player );
		else if( SHA256( GetTok( arguments, " ", 1 ) ) != status[player.ID].pass) MessagePlayer( "[#FF0000]Error:[#FFFFFF] Your password needs to be different from your previous one.", player );
		else
		{
